...
from form_Magazzino import formAggiungiModificaNomeElemento
    ...
    def btEliminaNomeClicked(self):
        ...
        if selectedRow == -1:
            ...
            if button == QMessageBox.Yes:
                ...
                risultato = GestoreDBMagazzino.eseguiQuery(query)
                if risultato != "corretto":
                    dlg = QMessageBox()
                    dlg.setWindowTitle("Errore")
                    dlg.setText(f"Si è presentato un errore: \n{risultato}\n l'elemento non è stato rimosso")
                    dlg.setStandardButtons(QMessageBox.Ok)
                    dlg.setIcon(QMessageBox.Critical)
                    dlg.exec()
            self.load_data_tabella()
