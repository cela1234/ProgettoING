...
from form_Magazzino import gestioneNomi
from form_Magazzino import formAggiungiModificaElementoMagazzino
...
class Ui_formMagazzino(object):
    ...
    def btModificaClicked(self):
        # Verifica se è selezionata una riga
        selected_row = self.tableMagazzino.currentRow()
        if selected_row == -1:
            QtWidgets.QMessageBox.warning(None, "Errore", "Seleziona una riga prima di modificare.")
            return

        # Disabilita l'interfaccia principale
        self.fMagazzino.setEnabled(False)

        # Crea la finestra di dialogo per la modifica
        ...
        # Riempie i campi del form con i dati della riga selezionata
        dataStringa = self.tableMagazzino.item(selected_row, 4).text()
        dataScadenzaDate = datetime.strptime(dataStringa, '%Y-%m-%d').date()
        self.uiModifica.dateEditScadenza.setDate(dataScadenzaDate)
        self.uiModifica.numberQuantita.setValue(float(self.tableMagazzino.item(selected_row, 3).text()))
        PrezzoSenzaEuro = self.tableMagazzino.item(selected_row, 2).text().replace("€", "")
        self.uiModifica.numberPrezzo.setValue(float(PrezzoSenzaEuro))
        self.uiModifica.txtFornitore.setText(self.tableMagazzino.item(selected_row, 5).text())

        # Esegue la finestra di dialogo per la modifica
        self.formModifica.exec_()

        # Riabilita l'interfaccia principale e ricarica i dati
        self.fMagazzino.setEnabled(True)
        self.load_data_tabella()
